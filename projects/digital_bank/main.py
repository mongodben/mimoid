...
def generate_summary_report(connection_string: str):
    ...
    try:
        ...
        if sample_customer:
            ...
            print(
                f"  Alternative Data: {', '.join(alt_data_sources) if alt_data_sources else 'None'}"
            )

        # Show loan application approval rates
        total_applications = db.loan_applications.count_documents({})
        approved_applications = db.loan_applications.count_documents(
            {"status": "approved"}
        )
        ...
        credit_scores = list(
            db.credit_scores.find(
                {}, {"alternative_data_weight": 1, "confidence_level": 1}
            ).limit(100)
        )

        if credit_scores:
            avg_alt_weight = sum(
                score.get("alternative_data_weight", 0) for score in credit_scores
            ) / len(credit_scores)
            avg_confidence = sum(
                score.get("confidence_level", 0) for score in credit_scores
            ) / len(credit_scores)
            print(f"\n🤖 AI Credit Scoring:")
