...
from db_schema import (
    database_schema, ApplicationStatus, LoanStatus, PaymentStatus,
    PaymentMethod, RiskLevel, DataSourceType
)
...
class NeoLendBankSeeder(DatabaseSeeder):
    def __init__(self, connection_string: str):
        ...
        self.loan_amounts = [(100, 0.3), (250, 0.25), (500, 0.2), (750, 0.15), (1000, 0.07), (1500, 0.03)]
        self.loan_purposes = [
            "Business expansion", "Equipment repair", "Working capital", "Inventory purchase",
            "Medical expenses", "Education", "Home improvement", "Emergency funds",
            "Debt consolidation", "Vehicle repair"
        ]
        
        self.employment_types = ["Self-employed", "Small business owner", "Freelancer", "Part-time worker", "Unemployed", "Student"]
    ...
    def generate_alternative_data(self):
        ...
        if random.random() < 0.9:  # 90% have device data
            device_data = {
                ...
                "device_model": random.choice(["Samsung Galaxy", "iPhone", "Xiaomi", "Oppo", "Vivo"]),
        ...
        if random.random() < 0.4:  # 40% have network data
            network_data = {
                ...
                "frequent_contact_patterns": random.choice(["Family", "Business", "Mixed"])
            }
        
        return social_media_data, behavioral_data, device_data, network_data

    def calculate_credit_score(self, customer_data, social_data, behavioral_data, device_data):
        ...
        # Behavioral factors
        if behavioral_data:
            completion_time = behavioral_data.get('loan_form_completion_time_seconds', 600)
    ...
    def seed_customers(self, count: int):
        ...
        for i in range(count):
            social_data, behavioral_data, device_data, network_data = self.generate_alternative_data()
            
            # Generate basic customer info
            first_name = self.fake.first_name()
            last_name = self.fake.last_name()
            monthly_income = random.uniform(200, 2000) if random.random() < 0.6 else None
            ...
            credit_score = self.calculate_credit_score(customer_data, social_data, behavioral_data, device_data)
            ...
            customer = {
                ...
                'national_id': f"ID{random.randint(1000000000, 9999999999)}" if random.random() < 0.3 else None,
                'first_name': first_name,
                'last_name': last_name,
                'date_of_birth': self.fake.date_time_between(start_date='-65y', end_date='-18y') if random.random() < 0.5 else None,
                ...
                'monthly_income': monthly_income,
                'employment_status': employment_status,
                'employer_name': self.fake.company() if employment_status not in ['Unemployed', 'Student'] and random.random() < 0.6 else None,
                ...
                'fraud_score': random.uniform(0, 20),  # Most customers have low fraud scores
                'registration_date': self.fake.date_time_between(start_date='-3y', end_date='-1d'),
                'last_activity': self.fake.date_time_between(start_date='-30d', end_date='now') if random.random() < 0.8 else None,
                'kyc_completed': random.choice([True, True, True, False]),  # 75% completed KYC
                'kyc_completion_date': self.fake.date_time_between(start_date='-2y', end_date='now') if random.random() < 0.7 else None,
                'data_consent': {
                ...
                'created_at': self.fake.date_time_between(start_date='-3y', end_date='-1d'),
                'updated_at': self.fake.date_time_between(start_date='-30d', end_date='now') if random.random() < 0.6 else None,
                'data_sources': [source.value for source in random.sample(list(DataSourceType), k=random.randint(2, 4))]
            }
    ...
    def seed_loan_applications(self, count: int):
        ...
        if count > len(self.customer_ids) * 2:
            print(f"Warning: Generating {count} applications for {len(self.customer_ids)} customers")
        ...
        for i in range(count):
            ...
            if credit_score >= 650:
                status = random.choices([ApplicationStatus.APPROVED, ApplicationStatus.REJECTED], weights=[0.8, 0.2])[0]
            elif credit_score >= 500:
                status = random.choices([ApplicationStatus.APPROVED, ApplicationStatus.REJECTED], weights=[0.5, 0.5])[0]
            else:
                status = random.choices([ApplicationStatus.APPROVED, ApplicationStatus.REJECTED], weights=[0.2, 0.8])[0]
            
            submitted_date = self.fake.date_time_between(start_date='-2y', end_date='now')
            ...
            application = {
                ...
                'supporting_documents': [
                    {'type': 'id_photo', 'status': 'verified', 'uploaded_at': submitted_date},
                    {'type': 'selfie', 'status': 'verified', 'uploaded_at': submitted_date}
                ] if random.random() < 0.8 else [],
                'device_fingerprint': {
                ...
                'reviewed_at': submitted_date + timedelta(hours=random.randint(1, 48)) if status != ApplicationStatus.SUBMITTED else None,
                'decision_date': submitted_date + timedelta(hours=random.randint(2, 72)) if status in [ApplicationStatus.APPROVED, ApplicationStatus.REJECTED] else None,
                'credit_score': credit_score + random.randint(-50, 50),  # Slight variation from customer score
                'risk_assessment': {
                ...
                'decision_factors': [
                    ...
                    {'factor': 'Income Verification', 'weight': 0.2, 'value': random.uniform(0.5, 1.0)},
                    {'factor': 'Alternative Data', 'weight': 0.25, 'value': random.uniform(0.3, 0.9)},
                    {'factor': 'Device Trust', 'weight': 0.15, 'value': random.uniform(0.6, 1.0)},
                    {'factor': 'Application Quality', 'weight': 0.1, 'value': random.uniform(0.7, 1.0)}
                ],
                'model_version': random.choice(['v2.1.0', 'v2.2.0', 'v2.3.0']),
                'approved_amount': requested_amount * random.uniform(0.8, 1.0) if status == ApplicationStatus.APPROVED else None,
                'approved_term_days': random.choice([30, 45, 60]) if status == ApplicationStatus.APPROVED else None,
                'interest_rate': random.uniform(15, 35) if status == ApplicationStatus.APPROVED else None,
                ...
                ]) if status == ApplicationStatus.REJECTED else None,
    ...
    def seed_loans(self, count: int):
        ...
        if len(approved_apps) < count:
            print(f"Warning: Only {len(approved_apps)} approved applications available for {count} loans")
        ...
        for i in range(count):
            app = approved_apps[i] if i < len(approved_apps) else random.choice(approved_apps)
            ...
            disbursed_date = app['decision_date'] + timedelta(hours=random.randint(4, 48))
            ...
            if status == LoanStatus.PAID_OFF:
                ...
                closed_date = disbursed_date + timedelta(days=random.randint(term_days-10, term_days))
            elif status in [LoanStatus.DEFAULTED, LoanStatus.CHARGED_OFF, LoanStatus.IN_COLLECTIONS]:
                ...
                actual_progress = expected_progress * random.uniform(0.8, 1.2)  # Some variance
            ...
            loan = {
                ...
                'due_date': due_date,
                'payment_schedule': self.generate_payment_schedule(principal, total_amount, term_days, disbursed_date),
                'status': status.value,
                ...
                'fees_paid': round(random.uniform(0, 20) if status != LoanStatus.PAID_OFF else 0, 2),
                'outstanding_balance': round(outstanding_balance, 2),
                'days_past_due': days_past_due,
                'payment_count': int(payment_progress * random.randint(term_days//7, term_days//3)),
                'missed_payments': random.randint(0, 3) if status != LoanStatus.PAID_OFF else 0,
                'last_payment_date': disbursed_date + timedelta(days=int(payment_progress * term_days)) if total_paid > 0 else None,
                'current_risk_level': self.assess_loan_risk(days_past_due, payment_progress).value,
                'in_collections': status == LoanStatus.IN_COLLECTIONS,
                'collections_start_date': due_date + timedelta(days=30) if status in [LoanStatus.IN_COLLECTIONS, LoanStatus.DEFAULTED] else None,
                'closed_at': closed_date,
                'closure_reason': status.value if closed_date else None,
                'created_at': disbursed_date,
                'updated_at': self.fake.date_time_between(start_date=disbursed_date, end_date='now') if random.random() < 0.7 else None
            }
            loans.append(loan)
        
        self.db.loans.insert_many(loans)
        self.loan_ids = [loan['_id'] for loan in loans]
        self.active_loan_ids = [loan['_id'] for loan in loans if loan['status'] == LoanStatus.ACTIVE.value]
    ...
    def generate_payment_schedule(self, principal, total_amount, term_days, start_date):
        ...
        for day in range(term_days):
            schedule.append({
                ...
                'interest_portion': round((total_amount - principal) / term_days, 2)
            })
    ...
    def seed_payments(self, count: int):
        ...
        for i in range(count):
            ...
            payment_method = random.choices(
                list(PaymentMethod),
                weights=[0.1, 0.4, 0.3, 0.15, 0.05]  # bank_transfer, digital_wallet, mobile_money, cash_agent, debit_card
            )[0]
            ...
            processed_date = scheduled_date + timedelta(minutes=random.randint(1, 1440)) if status == PaymentStatus.COMPLETED else None
            
            # Calculate payment breakdown
            remaining_principal = max(0, loan['principal_amount'] - loan['principal_paid'])
            ...
            payment = {
                ...
                'payment_reference': f"PAY{random.randint(100000, 999999)}" if random.random() < 0.8 else None,
                ...
                'payment_processor': random.choice(['GoPay', 'OVO', 'DANA', 'BankTransfer', 'Indomaret']) if payment_method != PaymentMethod.CASH_AGENT else None,
                'transaction_id': f"TXN{uuid.uuid4().hex[:12].upper()}" if status == PaymentStatus.COMPLETED else None,
                'failure_reason': random.choice([
                    'Insufficient funds',
                    'Invalid account',
                    'Network timeout',
                    'Declined by bank'
                ]) if status == PaymentStatus.FAILED else None,
    ...
    def seed_credit_scores(self, count: int):
        ...
        for i in range(count):
            ...
            credit_score = {
                ...
                'data_sources_used': [source.value for source in random.sample(list(DataSourceType), k=random.randint(2, 5))],
                ...
                'application_id': random.choice(self.application_ids) if random.random() < 0.4 and self.application_ids else None,
    ...
    def seed_collection_cases(self, count: int):
        ...
        overdue_loans = list(self.db.loans.find({
            '$or': [
        ...
        if len(overdue_loans) < count:
            print(f"Warning: Only {len(overdue_loans)} overdue loans available for {count} collection cases")
            count = len(overdue_loans)
        
        collection_cases = []
        
        for i in range(count):
            loan = overdue_loans[i] if i < len(overdue_loans) else random.choice(overdue_loans)
            
            opened_date = loan.get('collections_start_date') or (loan['due_date'] + timedelta(days=30))
            case_number = f"COL{datetime.now().year}{str(i).zfill(5)}{random.randint(10, 99)}"
            ...
            for attempt in range(num_attempts):
                contact_attempts.append({
                    ...
                    'outcome': random.choice(['no_answer', 'promised_payment', 'payment_plan', 'disputed', 'contact_made']),
            ...
            if random.random() < 0.4:  # 40% make some payment during collection
                ...
                for payment in range(num_payments):
                    payments_during_collection.append({
                        ...
                        'amount': random.uniform(10, loan['outstanding_balance'] * 0.5),
                        'method': random.choice(['digital_wallet', 'bank_transfer', 'cash_agent'])
                    })
            ...
            collection_case = {
                ...
                'case_status': random.choice(['open', 'in_progress', 'settled', 'charged_off']),
                ...
                'collection_strategy': random.choice(['phone_first', 'sms_campaign', 'field_visit', 'legal_notice']),
                'assigned_agent': f"agent_{random.randint(1, 20)}" if random.random() < 0.8 else None,
                'assigned_date': opened_date + timedelta(days=random.randint(0, 7)) if random.random() < 0.8 else None,
                'resolution_date': opened_date + timedelta(days=random.randint(30, 180)) if random.random() < 0.3 else None,
                'resolution_type': random.choice(['paid_in_full', 'payment_plan', 'partial_settlement', 'charged_off']) if random.random() < 0.3 else None,
                'recovery_amount': sum(p['amount'] for p in payments_during_collection) if payments_during_collection else 0,
                'created_at': opened_date,
                'updated_at': self.fake.date_time_between(start_date=opened_date, end_date='now') if random.random() < 0.8 else None,
                'notes': [
                    {
                        'date': opened_date + timedelta(days=random.randint(0, 30)),
                        'agent': f"agent_{random.randint(1, 20)}",
                        'note': self.fake.paragraph(),
                        'category': random.choice(['contact', 'payment', 'dispute', 'update'])
                    }
    ...
    def seed_compliance_records(self, count: int):
        ...
        for i in range(count):
            ref_id, ref_type = random.choice(all_refs) if all_refs else (ObjectId(), 'customer')
            ...
            if 'Lending' in regulation:
                compliance_data = {
                    ...
                    'customer_category': random.choice(['unbanked', 'underbanked', 'returning']),
            ...
            compliance_record = {
                ...
                'report_submitted': random.choice([True, False]) if random.choice([True, False]) else False,
                'submission_date': self.fake.date_time_between(start_date=period_date, end_date='now') if random.choice([True, False]) else None,
                'created_at': self.fake.date_time_between(start_date=period_date, end_date='now'),
                'created_by': random.choice(['compliance_system', 'audit_bot', 'regulatory_agent']),
                'data_hash': f"hash_{uuid.uuid4().hex[:16]}" if random.random() < 0.8 else None
            }
    ...
    def validate_seed_data(self):
        ...
        try:
            ...
            if invalid_apps > 0:
                ...
                validation_results['errors'].append(f"Found {invalid_apps} applications with invalid customer references")
            ...
            if invalid_loans > 0:
                validation_results['referential_integrity'] = False
                validation_results['errors'].append(f"Found {invalid_loans} loans with invalid references")
        
        except Exception as e:
            validation_results['errors'].append(f"Error checking referential integrity: {e}")
        ...
        for collection_name, collection_schema in self.database_schema.collections.items():
            ...
            for index_def in collection_schema.indexes:
                if index_def.name not in existing_indexes:
                    ...
                    validation_results['errors'].append(f"Index '{index_def.name}' not found in collection '{collection_name}'")
