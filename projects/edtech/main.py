...
def validate_environment():
    ...
    for var, value in optional_vars.items():
        masked_value = value if 'URI' not in var else value.split('@')[-1] if '@' in value else value
...
def generate_sample_queries():
    ...
    try:
        ...
        print(f"    • Cultural diversity index: {diversity_stats['diversity_index']:.2f}")
        ...
        print(f"    • Total ProUni applications: {funding_stats['prouni_applications']:,}")
        print(f"    • Average processing time: {funding_stats['avg_processing_days']:.1f} days")
        ...
        print(f"    • Average documents per application: {document_stats['avg_docs_per_app']:.1f}")
        print(f"    • Verification completion rate: {document_stats['verification_rate']:.1%}")
        ...
        for institution in institution_stats[:5]:  # Top 5 institutions
            print(f"    • {institution['name']}: {institution['students']:,} students, {institution['avg_gpa']:.1f} avg GPA")
