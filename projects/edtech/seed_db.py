...
class BrazilianEdTechSeeder(DatabaseSeeder):
    ...
    def __init__(self, connection_string: str = "mongodb://localhost:27017"):
        ...
        self.funding_programs = [
            ...
            (
                "ProUni",
                "University for All Program - Scholarships for low-income students",
            ),
    ...
    def seed_institutions(self, count: int = 12):
        ...
        for _ in range(count):
            ...
            institution = Institution(
                ...
                # Educational details
                accreditation_level=random.choice(
                    ["Credenciada", "Recredenciada", "Em Supervisão"]
                ),
                ...
                # Academic offerings
                degree_levels=random.sample(
                    ["Graduação", "Pós-graduação", "Mestrado", "Doutorado"],
                    k=random.randint(2, 4),
                ),
                academic_areas=[
                    self.fake.academic_program() for _ in range(random.randint(5, 15))
                ],
        ...
        print(
            f"  ✅ Created {len(institutions)} institutions with diverse Brazilian names"
        )
    ...
    def seed_students(self, count: int = 5000):
        ...
        for batch_start in range(0, count, batch_size):
            ...
            for i in range(batch_start, batch_end):
                ...
                birth_date = self.fake.date_time_between(
                    start_date="-30y", end_date="-16y"
                )
                enrollment_date = self.fake.date_time_between(
            ...
            print(
                f"  📚 Created batch {batch_start // batch_size + 1}: {len(batch_students)} students"
            )

        print(
            f"  ✅ Created {len(students)} students with diverse Brazilian naming patterns"
        )
    ...
    def seed_applications(self, count: int = 15000):
        ...
        for batch_start in range(0, count, batch_size):
            ...
            for i in range(batch_start, batch_end):
                ...
                funding_program, program_description = random.choice(
                    self.funding_programs[:2]
                )  # Focus on FIES/ProUni

                program_id = (
                    random.choice(self.program_ids)
                    if hasattr(self, "program_ids") and self.program_ids
                ...
                application = Application(
                    ...
                    + timedelta(days=random.randint(45, 90))
                    ...
                    application_form={
                        ...
                        "high_school_type": random.choice(
                            ["public", "private", "mixed"]
                        ),
                        "is_cadunico_registered": random.choice([True, False]),
                    },
                    # Required financial information
                    family_income_declared=random.uniform(
                        1000, 8000
                    ),  # Monthly family income in BRL
                    ...
                    required_documents=random.sample(
                        list(DocumentType), k=random.randint(5, 11)
                    ),
                    ...
                    fies_protocol=f"FIES{random.randint(100000, 999999)}"
                    if funding_program.lower() == "fies"
                    else None,
                    prouni_protocol=f"PROUNI{random.randint(100000, 999999)}"
                    if funding_program.lower() == "prouni"
                    else None,
                    government_status=random.choice(
                        ["pending", "submitted", "approved", "rejected"]
                    ),
                    # Verification statuses
                    data_verification_status=random.choice(
                        ["pending", "verified", "requires_review"]
                    ),
                    ...
                    approved_amount=random.uniform(15000, 120000)
                    if random.random() < 0.6
                    else None,
                    # Status and metadata
                    created_at=application_date,
                    updated_at=self.fake.date_time_between(
                        start_date=application_date, end_date="now"
                    ),
                )
            ...
            print(
                f"  📋 Created batch {batch_start // batch_size + 1}: {len(batch_applications)} applications"
            )
    ...
    def seed_documents(self, count: int = 180000):
        ...
        for batch_start in range(0, count, batch_size):
            ...
            for i in range(batch_start, batch_end):
                ...
                doc_type = random.choices(
                    ...
                    weights=[
                        ...
                        12,
                        5,
                    ],  # Higher weights for common docs (11 total)
                    k=1,
                )[0]
                ...
                document = Document(
                    ...
                    verification_date=self.fake.date_time_between(
                        start_date=submission_date, end_date="now"
                    )
                    if random.random() < 0.8
                    else None,
                    verified_by=random.choice(self.staff_ids)
                    if len(self.staff_ids) > 0
                    else None,
                    # Review information
                    review_notes=f"Document {doc_type} verification"
                    if random.random() < 0.5
                    else None,
                    # Document processing
                    ocr_extracted_text=f"Extracted text from {doc_type} document"
                    if random.random() < 0.6
                    else None,
                    extracted_data={
                    ...
                    digital_signature=f"signature_{random.randint(1000, 9999)}"
            ...
            print(
                f"  📄 Created batch {batch_start // batch_size + 1}: {len(batch_documents)} documents"
            )
    ...
    def seed_staff(self, count: int = 800):
        ...
        for i in range(count):
            ...
            staff_member = Staff(
                ...
                # Employment details
                hire_date=self.fake.date_time_between(
                    start_date="-15y", end_date="-1m"
                ),
                ...
                maximum_applications=random.randint(10, 50)
                if role in [StaffRole.ADMINISTRATOR, StaffRole.ADMISSIONS_OFFICER]
                else None,
                # Status
                created_at=datetime.utcnow(),
                updated_at=datetime.utcnow(),
        ...
        print(
            f"  ✅ Created {len(staff_members)} staff members with diverse Brazilian backgrounds"
        )
    ...
    def seed_programs(self, count: int = 100):
        ...
        for i in range(count):
            ...
            program = Program(
                ...
                degree_type=random.choice(
                    ["Bacharel", "Licenciatura", "Tecnólogo", "Mestrado", "Doutorado"]
                ),
                field_of_study=program_name,
                specialization=f"Especialização em {program_name}"
                if random.random() < 0.3
                else None,
                # Required fields
                ...
                accreditation_status=random.choice(
                    ["accredited", "provisional", "under_review"]
                ),
    ...
    def seed_courses(self, count: int = 800):
        ...
        for i in range(count):
            ...
            primary_instructor = (
                random.choice(self.staff_ids) if self.staff_ids else self.generate_id()
            )
            ...
            course = Course(
                ...
                prerequisites=random.sample(
                    [f"CURSO{j:04d}" for j in range(1, i)], k=random.randint(0, 3)
                )
                if i > 10
                else [],
                learning_objectives=[
                ...
                assessment_methods=[
                    {
                        "type": "provas",
                        "weight": 0.4,
                        "description": "Avaliações escritas",
                    },
                    {
                        "type": "trabalhos",
                        "weight": 0.3,
                ...
                reading_list=[
                    {
                        "title": f"Livro de {program_name} - Volume 1",
                        "author": "Autor Brasileiro",
                        "required": True,
                    },
                    {
                        "title": f"Manual de {program_name} - Edição Brasileira",
                        "author": "Especialista",
                        "required": False,
                    },
                ],
    ...
    def seed_enrollments(self, count: int = 25000):
        ...
        for batch_start in range(0, count, batch_size):
            ...
            for i in range(batch_start, batch_end):
                ...
                enrollment = Enrollment(
                    ...
                    payment_status=random.choice(
                        ["paid", "pending", "overdue", "financial_aid"]
                    ),
                    # Completion details
                    completion_date=enrollment_date
                    + timedelta(days=random.randint(120, 180))
                    if random.random() < 0.7
                    else None,
                    final_grade=random.uniform(6.0, 10.0)
                    if random.random() < 0.8
                    else None,
                    ...
                    updated_at=self.fake.date_time_between(
                        start_date=enrollment_date, end_date="now"
                    ),
                )
            ...
            print(
                f"  📖 Created batch {batch_start // batch_size + 1}: {len(batch_enrollments)} enrollments"
            )
    ...
    def seed_assessments(self, count: int = 50000):
        ...
        for batch_start in range(0, count, batch_size):
            ...
            for i in range(batch_start, batch_end):
                ...
                # Calculate scores
                points_earned = (
                    random.uniform(4.0, 10.0) if random.random() < 0.85 else 0.0
                )
                points_possible = 10.0
                percentage_score = (
                    (points_earned / points_possible) if points_possible > 0 else 0
                )  # Decimal 0-1, not percentage
                ...
                assessment = Assessment(
                    ...
                    if hasattr(self, "enrollment_ids") and self.enrollment_ids
                    else self.generate_id(),  # Required field
                    # Assessment details
                    ...
                    submission_date=self.fake.date_time_between(
                        ...
                        end_date=due_date + timedelta(days=2),
                    )
                    ...
                    weight_percentage=random.uniform(
                        0.1, 0.5
                    ),  # Required field (decimal 0-1, not percentage)
                    ...
                    # Submission details
                    submission_method=random.choice(
                        ["online_upload", "physical_submission", "oral_presentation"]
                    ),
            ...
            print(
                f"  ✏️  Created batch {batch_start // batch_size + 1}: {len(batch_assessments)} assessments"
            )

        print(
            f"  ✅ Created {len(assessments)} assessments with Brazilian naming patterns"
        )
        ...
        for i in range(count):
            ...
            content = Content(
                ...
                category=random.choice(
                    ["lecture", "assignment", "reading", "discussion", "assessment"]
                ),
                ...
                visibility=random.choice(
                    ["public", "institution", "course", "private"]
                ),
                ...
                # Publishing details
                author=random.choice(self.staff_ids)
                if self.staff_ids
                else self.generate_id(),
                institution_id=random.choice(self.institution_ids),
    ...
    def seed_financial_aid(self, count: int = 8000):
        ...
        for batch_start in range(0, count, batch_size):
            ...
            for i in range(batch_start, batch_end):
                ...
                award_date = self.fake.date_time_between(
                    start_date="-2y", end_date="now"
                )
                ...
                financial_aid = FinancialAid(
                    ...
                    aid_type=random.choice(
                        ["scholarship", "loan", "grant", "work_study"]
                    ),  # Required field
                    ...
                    end_date=award_date
                    + timedelta(
                        days=random.randint(365, 2190)
                    ),  # Required field (1-6 years)
                    ...
                    status_reason="Aid program requirements met"
                    if random.random() < 0.8
                    else "Under review",
                    government_approval_number=f"{program}_{random.randint(100000, 999999)}"
                    if program in ["FIES", "ProUni"]
                    else None,
                    government_status="approved"
                    if random.random() < 0.8
                    else "pending",
                    fies_contract_number=f"FIES_{random.randint(100000000, 999999999)}"
                    if program == "FIES"
                    else None,
                    # Metadata
                    created_at=award_date,
                    updated_at=self.fake.date_time_between(
                        start_date=award_date, end_date="now"
                    ),
                )
            ...
            print(
                f"  💳 Created batch {batch_start // batch_size + 1}: {len(batch_records)} aid records"
            )
    ...
    def run_seeding(self):
        ...
        total_documents = (
            12 + 800 + 100 + 5000 + 800 + 15000 + 180000 + 25000 + 50000 + 3000 + 8000
        )
        ...
        print()
        print(
            "🇧🇷 Ready to serve 2.4 million Brazilian students with diverse cultural backgrounds!"
        )
    ...
    def validate_seed_data(self):
        ...
        for collection_name in self.database_schema.collections.keys():
            ...
            if count == 0:
                raise ValueError(
                    f"Collection '{collection_name}' is empty after seeding"
                )
