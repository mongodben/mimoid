...
class Facility(BaseMongoDbDocumentSchema):
    ...
    facility_code: str = Field(..., max_length=20, description="Unique facility identifier")
    ...
    coordinates: Dict[str, float] = Field(default={}, description="Latitude and longitude")
    
    # Operational details
    annual_capacity_tons: int = Field(..., ge=0, description="Annual production capacity")
    operational_since: datetime = Field(...)
    
    # Quality certifications
    iso_certifications: List[str] = Field(default=[], description="ISO certifications held")
    quality_standards: List[str] = Field(default=[], description="Quality standards compliance")
...
class ProductionLine(BaseMongoDbDocumentSchema):
    ...
    product_types: List[ProductType] = Field(..., description="Types of products this line can produce")
    ...
    production_speed_mpm: float = Field(..., ge=0, description="Production speed in meters per minute")
    annual_capacity_tons: int = Field(..., ge=0)
    
    # Equipment configuration
    equipment_list: List[Dict[str, Any]] = Field(default=[], description="List of equipment and sensors")
    checkpoint_locations: List[Dict[str, Any]] = Field(default=[], description="Quality checkpoint positions")
    
    # Current status
    status: ProductionLineStatus = Field(default=ProductionLineStatus.OPERATIONAL)
    current_product: Optional[str] = Field(None, description="Currently producing product")
...
class Product(BaseMongoDbDocumentSchema):
    ...
    product_code: str = Field(..., max_length=50, description="Unique product identifier")
    ...
    chemical_composition: Dict[str, float] = Field(default={}, description="Element percentages")
    ...
    applicable_standards: List[str] = Field(default=[], description="Industry standards compliance")
    
    # Customer and market info
    target_markets: List[str] = Field(default=[], description="Geographic or industry markets")
    customer_specifications: Dict[str, Any] = Field(default={}, description="Customer-specific requirements")
...
class ProductionBatch(BaseMongoDbDocumentSchema):
    facility_id: PyObjectId = Field(..., description="Reference to facility")
    production_line_id: PyObjectId = Field(..., description="Reference to production line")
    ...
    raw_material_batches: List[Dict[str, Any]] = Field(default=[], description="Raw material batch references")
    heat_number: Optional[str] = Field(None, description="Steel heat number for traceability")
    
    # Process parameters
    process_parameters: Dict[str, float] = Field(default={}, description="Key process settings")
...
class QualityCheckpoint(BaseMongoDbDocumentSchema):
    ...
    checkpoint_id: str = Field(..., max_length=50, description="Unique checkpoint identifier")
    checkpoint_type: CheckpointType = Field(...)
    location_km: float = Field(..., ge=0, description="Position along production line in km")
    
    # Measurement data
    measurement_timestamp: datetime = Field(default_factory=datetime.utcnow)
    sensor_readings: Dict[str, float] = Field(..., description="Raw sensor measurements")
    ...
    chemical_analysis: Dict[str, float] = Field(default={}, description="Element concentrations")
    ...
    deviation_from_spec: Dict[str, float] = Field(default={}, description="Deviations from specifications")
    ...
    # Data quality
    confidence_level: float = Field(default=95.0, ge=0, le=100, description="Measurement confidence")
    calibration_date: Optional[datetime] = Field(None, description="Last equipment calibration")
...
class Defect(BaseMongoDbDocumentSchema):
    ...
    production_line_id: PyObjectId = Field(..., description="Reference to production line")
    batch_id: PyObjectId = Field(..., description="Reference to production batch")
    checkpoint_id: Optional[PyObjectId] = Field(None, description="Reference to quality checkpoint")
    
    # Defect identification
    defect_id: str = Field(..., max_length=50, description="Unique defect identifier")
    defect_type: DefectType = Field(...)
    defect_subtype: str = Field(..., max_length=100, description="Specific defect classification")
    
    # Location and timing
    detection_timestamp: datetime = Field(default_factory=datetime.utcnow)
    location_km: float = Field(..., ge=0, description="Position where defect was detected")
    ...
    # Detection method
    detection_method: str = Field(..., max_length=50, description="How defect was detected")
    automated_detection: bool = Field(default=True)
    ai_confidence: Optional[float] = Field(None, ge=0, le=100, description="AI detection confidence")
    
    # Image and analysis data
    image_path: Optional[str] = Field(None, description="Path to defect image")
    image_analysis: Dict[str, Any] = Field(default={}, description="AI image analysis results")
    
    # Root cause analysis
    probable_cause: Optional[str] = Field(None, max_length=500)
    process_correlation: Dict[str, Any] = Field(default={}, description="Process parameters at time of defect")
    ...
    # Quality decision
    material_disposition: str = Field(default="accept", max_length=50, description="Accept, rework, or scrap")
...
class TestResult(BaseMongoDbDocumentSchema):
    ...
    # Sample information
    sample_location: str = Field(..., max_length=200, description="Where sample was taken")
    sample_preparation: str = Field(..., max_length=500, description="Sample preparation method")
    ...
    specification_limits: Dict[str, Dict[str, float]] = Field(default={}, description="Min/max acceptable values")
    pass_fail_status: str = Field(..., max_length=20)
    
    # Statistical analysis
    measurement_uncertainty: Dict[str, float] = Field(default={}, description="Measurement uncertainties")
    confidence_interval: Dict[str, List[float]] = Field(default={}, description="Confidence intervals")
    
    # Compliance
    compliant_standards: List[str] = Field(default=[], description="Standards this result meets")
    deviations: List[Dict[str, Any]] = Field(default=[], description="Any deviations from specifications")
...
class CustomerSpecification(BaseMongoDbDocumentSchema):
    ...
    applicable_products: List[PyObjectId] = Field(..., description="References to products")
    ...
    chemical_requirements: Dict[str, Dict[str, float]] = Field(default={}, description="Element min/max requirements")
    mechanical_requirements: Dict[str, Dict[str, float]] = Field(default={}, description="Strength/hardness requirements")
    dimensional_tolerances: Dict[str, Dict[str, float]] = Field(default={}, description="Size tolerances")
    surface_requirements: Dict[str, str] = Field(default={}, description="Surface quality specs")
    
    # Testing requirements
    required_tests: List[TestMethod] = Field(default=[], description="Mandatory test methods")
    sampling_plan: Dict[str, Any] = Field(default={}, description="Sampling requirements")
    
    # Acceptance criteria
    acceptance_criteria: Dict[str, Any] = Field(..., description="Pass/fail criteria")
    quality_level: str = Field(..., max_length=50, description="Quality level requirement")
...
class ProductionLineCollectionSchema(BaseCollectionSchema):
    ...
    indexes: List[IndexDefinition] = [
        ...
        IndexDefinition(
            ...
            keys={"product_types": IndexDirection.ASCENDING}
        ),
        IndexDefinition(
