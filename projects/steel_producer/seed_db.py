...
from mimoid import DatabaseSeeder as BaseDatabaseSeeder, PyObjectId
...
class SteelProductionSeeder(BaseDatabaseSeeder):
    ...
    def seed_all_collections(self, num_records: Optional[Dict[str, int]] = None):
        ...
        line_ids = self.seed_production_lines(
            num_records["production_lines"], facility_ids
        )
        product_ids = self.seed_products(num_records["products"])
        batch_ids = self.seed_production_batches(
            num_records["production_batches"], facility_ids, line_ids, product_ids
        )

        # High-volume time-series data
        self.seed_quality_checkpoints(
            num_records["quality_checkpoints"], facility_ids, line_ids, batch_ids
        )
        self.seed_defects(num_records["defects"], facility_ids, line_ids, batch_ids)
        self.seed_test_results(
            num_records["test_results"], facility_ids, batch_ids, product_ids
        )
        self.seed_customer_specifications(
            num_records["customer_specifications"], product_ids
        )
    ...
    def seed_facilities(self, count: int) -> List[ObjectId]:
        ...
        for i in range(count):
            ...
            facility = {
                ...
                "operational_since": self.fake.date_time_between(
                    start_date="-30y", end_date="-2y"
                ),
                "iso_certifications": random.sample(
                    ["ISO 9001", "ISO 14001", "ISO 45001", "ISO 50001"],
                    k=random.randint(2, 4),
                ),
                "quality_standards": random.sample(
                    ["ASTM", "EN", "JIS", "GB", "API"], k=random.randint(2, 4)
                ),
                ...
                "is_active": True,
                "last_audit_date": self.fake.date_time_between(
    ...
    def seed_production_lines(
        self, count: int, facility_ids: List[ObjectId]
    ) -> List[ObjectId]:
        ...
        for i in range(count):
            ...
            checkpoint_locations = [
                ...
                },
                {
                    ...
                    "equipment": "spectrometer_1",
                },
                {
                    "position_km": 2.0,
                    "type": "final_inspection",
                    "equipment": "multi_sensor_1",
                },
            ]
            ...
            line = {
                ...
                "product_types": random.sample(
                    list(ProductType), k=random.randint(2, 4)
                ),
                ...
                "status": random.choice(
                    [ProductionLineStatus.OPERATIONAL] * 8
                    + [ProductionLineStatus.MAINTENANCE]
                ),
                "current_product": random.choice(self.steel_grades)
                if random.random() > 0.2
                else None,
                "current_speed_mpm": random.uniform(6.0, 12.0)
                if random.random() > 0.3
                else None,
                "current_temperature_c": random.uniform(1100, 1250)
                if random.random() > 0.3
                else None,
                "efficiency_percent": random.uniform(78, 96),
                "quality_score": random.uniform(92, 99.5),
                "downtime_hours_monthly": random.uniform(8, 48),
                "last_maintenance_date": self.fake.date_time_between(
                    start_date="-3m", end_date="-1w"
                ),
                "next_maintenance_date": self.fake.date_time_between(
                    start_date="now", end_date="+2m"
                ),
                "created_at": datetime.utcnow(),
                "updated_at": None,
            }
    ...
    def seed_products(self, count: int) -> List[ObjectId]:
        ...
        for i in range(count):
            ...
            product = {
                ...
                "grade": grade,
                "nominal_thickness_mm": random.uniform(1.0, 20.0)
                if product_type
                in [ProductType.HOT_ROLLED_COIL, ProductType.COLD_ROLLED_COIL]
                else None,
                "nominal_width_mm": random.choice([1000, 1200, 1500, 1800, 2000])
                if "coil" in product_type.value
                else None,
                "nominal_length_mm": random.randint(6000, 12000)
                if product_type == ProductType.STEEL_PLATE
                else None,
                ...
                "surface_quality": random.choice(
                    ["commercial", "drawing", "structural", "exposed"]
                ),
                "tolerance_class": random.choice(["normal", "special", "precision"]),
                "application_areas": random.sample(
                    applications, k=random.randint(2, 5)
                ),
                "applicable_standards": random.sample(
                    ["ASTM A36", "EN 10025", "JIS G3101", "API 5L"],
                ...
                "development_stage": random.choice(
                    ["production"] * 8 + ["development", "pilot"]
                ),
                "created_at": datetime.utcnow(),
                "updated_at": None,
            }
    ...
    def seed_production_batches(
        ...
        for i in range(count):
            ...
            production_start = self.fake.date_time_between(
            ...
            actual_quantity = planned_quantity * random.uniform(
                0.92, 1.08
            )  # Some variation
            ...
            batch = {
                ...
                "raw_material_batches": [
                    {
                        "material": "iron_ore",
                        "batch_id": f"IO-{random.randint(1000, 9999)}",
                    },
                    {"material": "coal", "batch_id": f"C-{random.randint(1000, 9999)}"},
                    {
                        "material": "limestone",
                        "batch_id": f"LS-{random.randint(1000, 9999)}",
                    },
                ],
                ...
                "quality_grade": random.choice(
                    [QualityGrade.PRIME] * 6
                    + [QualityGrade.COMMERCIAL] * 3
                ...
                "customer_order_id": f"CO-{random.randint(10000, 99999)}"
                if random.random() > 0.3
                else None,
                "shipping_destination": self.fake.city()
                if random.random() > 0.2
                else None,
                "batch_status": random.choice(
                    ["completed"] * 7 + ["in_production", "quality_check"]
                ),
                "is_shipped": random.choice([True, False]),
                "shipped_date": production_end + timedelta(days=random.randint(1, 14))
    ...
    def seed_quality_checkpoints(
        ...
        while checkpoints_inserted < count:
            ...
            for i in range(current_batch_size):
                ...
                measurement_time = self.fake.date_time_between(
                    start_date="-1y", end_date="now"
                )
                ...
                checkpoint = {
                    ...
                    "thickness_mm": random.uniform(2.5, 15.0)
                    if checkpoint_type == CheckpointType.DIMENSIONAL_CHECK
                    else None,
                    "width_mm": random.uniform(1180, 1520)
                    if checkpoint_type == CheckpointType.DIMENSIONAL_CHECK
                    else None,
                    "temperature_c": random.uniform(1100, 1280)
                    if checkpoint_type == CheckpointType.TEMPERATURE_MEASUREMENT
                    else None,
                    "weight_kg": random.uniform(4800, 26000)
                    if checkpoint_type == CheckpointType.WEIGHT_CHECK
                    else None,
                    "chemical_analysis": self._generate_chemical_analysis()
                    if checkpoint_type == CheckpointType.CHEMICAL_ANALYSIS
                    else {},
                    "yield_strength_mpa": random.randint(280, 650)
                    if checkpoint_type == CheckpointType.MECHANICAL_TEST
                    else None,
                    "tensile_strength_mpa": random.randint(420, 750)
                    if checkpoint_type == CheckpointType.MECHANICAL_TEST
                    else None,
                    "hardness_hv": random.randint(160, 290)
                    if checkpoint_type == CheckpointType.MECHANICAL_TEST
                    else None,
                    "elongation_percent": random.uniform(18, 32)
                    if checkpoint_type == CheckpointType.MECHANICAL_TEST
                    else None,
                    ...
                    "calibration_date": self.fake.date_time_between(
    ...
    def seed_defects(
        ...
        for i in range(count):
            ...
            detection_time = self.fake.date_time_between(
                start_date="-1y", end_date="now"
            )
            ...
            defect = {
                ...
                "image_path": f"/images/defects/{i + 1:06d}.jpg"
                if random.random() > 0.4
                else None,
                "image_analysis": self._generate_image_analysis()
                ...
                "corrective_action": self._generate_corrective_action(defect_type)
                if random.random() > 0.2
                else None,
                "resolution_timestamp": detection_time
                + timedelta(hours=random.uniform(1, 48))
                if random.random() > 0.3
                else None,
                "resolved_by": self.fake.name() if random.random() > 0.3 else None,
                "material_disposition": random.choice(["accept", "rework", "scrap"])
                if severity != DefectSeverity.COSMETIC
                else "accept",
                "customer_notification_required": severity
                in [DefectSeverity.MAJOR, DefectSeverity.CRITICAL],
                "created_at": detection_time,
                "updated_at": detection_time + timedelta(hours=random.uniform(0.5, 24))
                if random.random() > 0.4
                else None,
            }
    ...
    def seed_test_results(
        ...
        for i in range(count):
            ...
            test_result = {
                ...
                "test_standard": random.choice(
                    ["ASTM E8", "ISO 6892", "EN 10025", "JIS Z2201"]
                ),
                "test_date": test_date,
                "laboratory": random.choice(
                    ["Central Lab", "Quality Lab A", "Quality Lab B", "External Lab"]
                ),
                "technician": self.fake.name(),
                "sample_location": random.choice(
                    ["head_end", "middle", "tail_end", "edge", "center"]
                ),
                ...
                "measurement_uncertainty": self._generate_measurement_uncertainty(
                    test_method
                ),
                "confidence_interval": self._generate_confidence_intervals(results),
                "compliant_standards": random.sample(
                    ["ASTM", "ISO", "EN", "JIS"], k=random.randint(1, 3)
                ),
                "deviations": self._generate_test_deviations(results, spec_limits),
                "certificate_number": f"CERT-{i + 1:06d}"
                if random.random() > 0.3
                else None,
                "certificate_issued": random.choice([True, False]),
                "certificate_valid_until": test_date + timedelta(days=365)
                if random.random() > 0.2
                else None,
    ...
    def seed_customer_specifications(self, count: int, product_ids: List[ObjectId]):
        ...
        for i in range(count):
            ...
            spec = {
                ...
                "effective_date": self.fake.date_time_between(
                    start_date="-2y", end_date="now"
                ),
                "expiry_date": self.fake.date_time_between(
                ...
                "surface_requirements": {
                    "surface_finish": random.choice(
                        ["mill_scale", "pickled", "oiled", "galvanized"]
                    ),
                    "surface_quality": random.choice(
                        ["commercial", "drawing", "structural"]
                    ),
                    "defect_limits": "As per specification document",
                },
                "required_tests": random.sample(
                    list(TestMethod), k=random.randint(2, 5)
                ),
                "sampling_plan": {
                    "sample_frequency": random.choice(
                        ["per_coil", "per_batch", "per_heat"]
                    ),
                    "sample_size": random.randint(1, 5),
                    "test_locations": random.choice(
                        ["head_tail", "random", "specified_positions"]
                    ),
                },
                "acceptance_criteria": {
        ...
        for collection_name, collection_schema in database_schema.collections.items():
            ...
            for index_def in collection_schema.indexes:
                try:
                    ...
                    logger.warning(
                        f"Failed to create index '{index_def.name}' on '{collection_name}': {e}"
                    )
    ...
    def _generate_sensor_readings(
        ...
        if checkpoint_type == CheckpointType.TEMPERATURE_MEASUREMENT:
            ...
            return {
                element: random.uniform(min_val, max_val)
                for element, (min_val, max_val) in list(self.chemical_elements.items())[
                    :5
                ]
            }
        else:
    ...
    def _generate_chemical_analysis(self) -> Dict[str, float]:
        ...
        return {
            element: round(random.uniform(min_val, max_val), 3)
            for element, (min_val, max_val) in self.chemical_elements.items()
        }
    ...
    def _generate_defect_size(
        self, severity: DefectSeverity, dimension: str
    ) -> Optional[float]:
    ...
    def _generate_corrective_action(self, defect_type: DefectType) -> str:
        ...
        actions = {
            DefectType.SURFACE_DEFECT: [
                "Roll maintenance scheduled",
                "Surface cleaning intensified",
                "Cooling system adjustment",
                "Scale removal optimization",
            ],
            DefectType.DIMENSIONAL_DEFECT: [
                "Gauge calibration",
                "Roll gap adjustment",
            ...
            DefectType.CHEMICAL_DEFECT: [
                ...
                "Alloy addition adjustment",
                "Melting parameters review",
            ...
            DefectType.COATING_DEFECT: [
                "Coating line maintenance",
                "Surface preparation improvement",
                "Coating material quality check",
                "Application parameters adjustment",
            ],
        }
    ...
    def _generate_specification_limits(
    ...
    def _determine_pass_fail(
        self, results: Dict[str, float], spec_limits: Dict[str, Dict[str, float]]
    ) -> str:
    ...
    def _generate_measurement_uncertainty(
        self, test_method: TestMethod
    ) -> Dict[str, float]:
        ...
        if test_method == TestMethod.TENSILE_TEST:
            ...
            return {"measurement_uncertainty_percent": random.uniform(1, 5)}
    ...
    def _generate_chemical_requirements(self) -> Dict[str, Dict[str, float]]:
        ...
        for element in random.sample(
            list(self.chemical_elements.keys()), k=random.randint(4, 8)
        ):
            ...
            requirements[element] = {
                "min": round(cust_min, 3),
                "max": round(cust_max, 3),
            }
        return requirements
    ...
    def _generate_mechanical_requirements(self) -> Dict[str, Dict[str, float]]:
        ...
        return {
            "yield_strength_mpa": {
                "min": random.randint(300, 450),
                "max": random.randint(500, 650),
            },
            "tensile_strength_mpa": {
                "min": random.randint(450, 550),
                "max": random.randint(600, 750),
            },
            "elongation_percent": {
                "min": random.uniform(18, 25),
                "max": random.uniform(30, 40),
            },
            "hardness_hv": {
                "min": random.randint(160, 200),
                "max": random.randint(250, 320),
            },
        }
    ...
    def _validate_referential_integrity(self):
        ...
        facility_ids = set(
            doc["_id"] for doc in self.db.facilities.find({}, {"_id": 1})
        )
        ...
        if invalid_line_facilities > 0:
            raise ValueError(
                f"Found {invalid_line_facilities} production lines with invalid facility references"
            )

        # Check production line references
        line_ids = set(
            doc["_id"] for doc in self.db.production_lines.find({}, {"_id": 1})
        )
        ...
        if invalid_batch_lines > 0:
            raise ValueError(
                f"Found {invalid_batch_lines} batches with invalid production line references"
            )
