...
class City(BaseMongoDbDocumentSchema):
    ...
    base_delivery_fee: float = Field(
        ..., ge=0, description="Base delivery fee in local currency"
    )
    max_delivery_radius_km: float = Field(
        ..., ge=0, description="Maximum delivery distance"
    )
    peak_hours: List[Dict[str, str]] = Field(
        default=[], description="Peak delivery time windows"
    )
...
class Customer(BaseMongoDbDocumentSchema):
    ...
    delivery_addresses: List[Dict[str, Any]] = Field(
        default=[], description="Saved delivery addresses"
    )
    favorite_cuisines: List[str] = Field(
        default=[], description="Preferred cuisine types"
    )
    dietary_restrictions: List[str] = Field(
        default=[], description="Allergies and dietary needs"
    )
...
class Restaurant(BaseMongoDbDocumentSchema):
    ...
    opening_hours: Dict[str, Dict[str, str]] = Field(
        ..., description="Weekly operating hours"
    )
    preparation_time_minutes: int = Field(
        ..., ge=5, le=120, description="Average order preparation time"
    )

    # Delivery settings
    delivery_radius_km: float = Field(
        ..., ge=0, le=50, description="Maximum delivery distance"
    )
    minimum_order_value: float = Field(default=0.0, ge=0)
    delivery_fee: float = Field(
        default=0.0, ge=0, description="Restaurant-specific delivery fee"
    )

    # Financial settings
    commission_rate: float = Field(
        ..., ge=0, le=1, description="Platform commission rate"
    )
    payment_method: str = Field(
        ..., max_length=50, description="How restaurant receives payments"
    )
    ...
    average_delivery_time: float = Field(
        default=0.0, ge=0, description="Average delivery time in minutes"
    )
...
class MenuItem(BaseMongoDbDocumentSchema):
    ...
    discounted_price: Optional[float] = Field(
        None, ge=0, description="Sale price if applicable"
    )

    # Availability
    is_available: bool = Field(default=True)
    availability_schedule: Optional[Dict[str, Any]] = Field(
        None, description="Time-based availability"
    )
    max_daily_quantity: Optional[int] = Field(
        None, ge=0, description="Daily stock limit"
    )
    ...
    customization_options: List[Dict[str, Any]] = Field(
        default=[], description="Size, extras, modifications"
    )
    ...
    popularity_score: float = Field(
        default=0.0, ge=0, description="Algorithm-based popularity"
    )
...
class Rider(BaseMongoDbDocumentSchema):
    ...
    vehicle_type: str = Field(
        ..., max_length=50, description="Motorcycle, bicycle, car, etc."
    )
    ...
    current_location: Optional[Dict[str, float]] = Field(
        None, description="Current GPS coordinates"
    )
    ...
    emergency_contact: Dict[str, str] = Field(
        default={}, description="Emergency contact information"
    )
...
class Order(BaseMongoDbDocumentSchema):
    ...
    order_number: str = Field(
        ..., max_length=50, description="Human-readable order number"
    )

    # Parties involved
    customer_id: PyObjectId = Field(..., description="Customer who placed the order")
    restaurant_id: PyObjectId = Field(
    ...
    items: List[Dict[str, Any]] = Field(
        ..., description="Ordered items with customizations"
    )
    subtotal: float = Field(
        ..., ge=0, description="Items total before fees and discounts"
    )
    ...
    delivery_address: Dict[str, str] = Field(
    ...
    estimated_preparation_time: int = Field(
        ..., ge=5, description="Minutes for preparation"
    )
    estimated_delivery_time: int = Field(
        ..., ge=10, description="Total estimated delivery time"
    )
    requested_delivery_time: Optional[datetime] = Field(
        None, description="Customer requested time"
    )

    # Status tracking
    status: OrderStatus = Field(default=OrderStatus.PENDING)
    status_history: List[Dict[str, Any]] = Field(
        default=[], description="Status change timeline"
    )
    ...
    promotion_codes: List[str] = Field(
        default=[], description="Applied promotion codes"
    )
    ...
    contact_preference: str = Field(
        default="phone", max_length=20, description="How to contact customer"
    )
    special_instructions: Optional[str] = Field(None, max_length=1000)

    # Ratings and feedback
    customer_rating: Optional[ReviewRating] = Field(
        None, description="Customer's rating of the order"
    )
...
class Delivery(BaseMongoDbDocumentSchema):
    ...
    pickup_coordinates: Dict[str, float] = Field(
        ..., description="Pickup GPS coordinates"
    )
    delivery_address: Dict[str, str] = Field(
        ..., description="Customer delivery address"
    )
    delivery_coordinates: Dict[str, float] = Field(
    ...
    location_updates: List[Dict[str, Any]] = Field(
        default=[], description="GPS tracking history"
    )

    # Route optimization
    optimized_route: Optional[List[Dict[str, float]]] = Field(
        None, description="Calculated optimal route"
    )
    ...
    # Delivery details
    delivery_instructions: Optional[str] = Field(None, max_length=500)
    delivery_proof: Optional[Dict[str, Any]] = Field(
        None, description="Photo or signature proof"
    )
    delivery_notes: Optional[str] = Field(
        None, max_length=500, description="Rider's delivery notes"
    )

    # Issues and resolution
    issues_reported: List[Dict[str, Any]] = Field(
        default=[], description="Delivery problems"
    )
...
class Payment(BaseMongoDbDocumentSchema):
    ...
    transaction_id: str = Field(
        ..., max_length=100, description="Payment gateway transaction ID"
    )
    gateway_reference: Optional[str] = Field(
        None, max_length=100, description="Gateway-specific reference"
    )
    external_transaction_id: Optional[str] = Field(
        None, max_length=100, description="External payment system ID"
    )
    ...
    card_last_four: Optional[str] = Field(
        None, max_length=4, description="Last 4 digits of card"
    )
    wallet_type: Optional[str] = Field(
        None, max_length=50, description="Digital wallet provider"
    )
    bank_reference: Optional[str] = Field(
        None, max_length=100, description="Bank transaction reference"
    )

    # Fees and charges
    processing_fee: float = Field(
        default=0.0, ge=0, description="Payment processing fee"
    )
    platform_fee: float = Field(default=0.0, ge=0, description="Platform service fee")

    # Refund information
    refund_amount: Optional[float] = Field(
        None, ge=0, description="Refunded amount if applicable"
    )
    ...
    risk_score: Optional[float] = Field(
        None, ge=0, le=100, description="Fraud risk assessment"
    )
...
class Review(BaseMongoDbDocumentSchema):
    ...
    restaurant_id: Optional[PyObjectId] = Field(
        None, description="Restaurant being reviewed"
    )
    ...
    service_rating: Optional[ReviewRating] = Field(
        None, description="Service quality rating"
    )
    delivery_rating: Optional[ReviewRating] = Field(
        None, description="Delivery experience rating"
    )

    # Review text
    title: Optional[str] = Field(None, max_length=200, description="Review title")
    comment: Optional[str] = Field(
        None, max_length=2000, description="Detailed review text"
    )
    ...
    flagged_content: List[str] = Field(
        default=[], description="Content moderation flags"
    )
    ...
    business_response: Optional[str] = Field(
        None, max_length=1000, description="Restaurant's response"
    )
...
class Promotion(BaseMongoDbDocumentSchema):
    ...
    total_usage_limit: Optional[int] = Field(
        None, ge=1, description="Total uses allowed"
    )
    per_customer_limit: int = Field(default=1, ge=1, description="Uses per customer")
    current_usage_count: int = Field(default=0, ge=0)

    # Targeting
    applicable_cities: List[PyObjectId] = Field(
        default=[], description="Cities where promotion is valid"
    )
    applicable_restaurants: List[PyObjectId] = Field(
        default=[], description="Specific restaurants"
    )
    customer_segments: List[str] = Field(
        default=[], description="Target customer segments"
    )
    ...
    auto_apply: bool = Field(
        default=False, description="Automatically apply if eligible"
    )

    # Performance tracking
    total_orders: int = Field(
        default=0, ge=0, description="Orders using this promotion"
    )
    total_discount_given: float = Field(
        default=0.0, ge=0, description="Total discount amount given"
    )
    conversion_rate: float = Field(
        default=0.0, ge=0, le=1, description="Usage to view ratio"
    )
...
class CityCollectionSchema(BaseCollectionSchema):
    ...
    indexes: List[IndexDefinition] = [
        ...
        IndexDefinition(name="coordinates_2dsphere", keys={"coordinates": "2dsphere"}),
    ]
...
class CustomerCollectionSchema(BaseCollectionSchema):
    ...
    indexes: List[IndexDefinition] = [
        ...
        IndexDefinition(
            name="phone_unique", keys={"phone": IndexDirection.ASCENDING}, unique=True
        ),
        IndexDefinition(
...
class DeliveryCollectionSchema(BaseCollectionSchema):
    ...
    indexes: List[IndexDefinition] = [
        ...
        IndexDefinition(
            name="pickup_coordinates_2dsphere", keys={"pickup_coordinates": "2dsphere"}
        ),
        IndexDefinition(
            name="delivery_coordinates_2dsphere",
            keys={"delivery_coordinates": "2dsphere"},
...
class PaymentCollectionSchema(BaseCollectionSchema):
    ...
    indexes: List[IndexDefinition] = [
        ...
        IndexDefinition(
            name="order_payments", keys={"order_id": IndexDirection.ASCENDING}
        ),
        IndexDefinition(
...
class PromotionCollectionSchema(BaseCollectionSchema):
    ...
    indexes: List[IndexDefinition] = [
        ...
        IndexDefinition(
            name="city_promotions", keys={"applicable_cities": IndexDirection.ASCENDING}
        ),
        IndexDefinition(
