...
from mimoid import DatabaseSeeder as BaseDatabaseSeeder
...
class FoodDeliverySeeder(BaseDatabaseSeeder):
    ...
    def seed_all_collections(self, num_records: Optional[Dict[str, int]] = None):
        ...
        order_ids = self.seed_orders(
            num_records["orders"], customer_ids, restaurant_ids, rider_ids
        )
        ...
        self.seed_reviews(
            num_records["reviews"], order_ids, customer_ids, restaurant_ids, rider_ids
        )
    ...
    def seed_cities(self, count: int) -> List[ObjectId]:
        ...
        for i, city_data in enumerate(selected_cities):
            city = {
                ...
                "is_active": True,
                "launch_date": self.fake.date_time_between(
    ...
    def seed_customers(self, count: int, city_ids: List[ObjectId]) -> List[ObjectId]:
        ...
        for i in range(count):
            ...
            customer = {
                ...
                "delivery_addresses": self._generate_delivery_addresses(
                    random.randint(1, 4)
                ),
                "favorite_cuisines": random.sample(
                    self.cuisine_types, k=random.randint(2, 5)
                ),
                ...
                "avatar_url": f"https://cdn.baemin.vn/avatars/{i + 1}.jpg"
                ...
                "last_order_date": self.fake.date_time_between(
    ...
    def seed_restaurants(self, count: int, city_ids: List[ObjectId]) -> List[ObjectId]:
        ...
        for i in range(count):
            ...
            total_reviews = int(
                total_orders * random.uniform(0.1, 0.4)
            )  # 10-40% review rate
            ...
            restaurant = {
                ...
                + (
                    random.sample(self.cuisine_types, k=random.randint(0, 2))
                ...
                "payment_method": random.choice(
                    ["bank_transfer", "cash_on_delivery", "digital_wallet"]
                ),
    ...
    def seed_menu_items(
        self, count: int, restaurant_ids: List[ObjectId]
    ) -> List[ObjectId]:
        ...
        for restaurant_id in restaurant_ids:
            restaurant_items_count = random.randint(
                max(1, items_per_restaurant - 5), items_per_restaurant + 5
            )
            ...
            for i in range(restaurant_items_count):
                ...
                discounted_price = (
                    base_price * random.uniform(0.8, 0.95)
                    if random.random() < 0.2
                    else None
                )
                ...
                menu_item = {
                    ...
                    "availability_schedule": self._generate_availability_schedule()
                    if random.random() < 0.3
                    else None,
                    "max_daily_quantity": random.randint(20, 100)
                    if random.random() < 0.4
                    else None,
                    ...
                    if "spicy" in dish_name.lower() or random.random() < 0.3
                    else None,
                    "is_vegetarian": "vegetarian" in dish_name.lower()
        ...
        for i in range(count):
            ...
            rider = {
                ...
                "vehicle_type": random.choice(
                    ["motorcycle", "bicycle", "scooter", "motorbike"]
                ),
                ...
                "current_location": self._generate_random_coordinates(city_ids)
                if random.random() > 0.3
                else None,
                ...
                "hourly_rate": random.uniform(25000, 40000)
                if random.random() > 0.5
                else None,  # VND per hour
                "per_delivery_rate": random.uniform(15000, 25000),  # VND per delivery
                "bonus_eligible": random.choice(
                    [True] * 8 + [False] * 2
                ),  # 80% bonus eligible
                "shift_start": self.fake.time() if random.random() > 0.3 else None,
                "shift_end": self.fake.time() if random.random() > 0.3 else None,
                "weekly_hours_target": random.randint(35, 48)
                if random.random() > 0.2
                else None,
                "is_active": random.choice([True] * 9 + [False]),  # 90% active
                "background_check_passed": random.choice(
                    [True] * 9 + [False]
                ),  # 90% passed
                "training_completed": random.choice(
                    [True] * 8 + [False] * 2
                ),  # 80% completed
                "emergency_contact": {
    ...
    def seed_orders(
        self,
        count: int,
        customer_ids: List[ObjectId],
        restaurant_ids: List[ObjectId],
        rider_ids: List[ObjectId],
    ) -> List[ObjectId]:
        ...
        while orders_inserted < count:
            ...
            for i in range(current_batch_size):
                ...
                rider_id = (
                    random.choice(rider_ids) if random.random() > 0.15 else None
                )  # 85% have assigned riders
                ...
                discount_amount = (
                    subtotal * random.uniform(0, 0.3) if random.random() < 0.3 else 0
                )
                total_amount = (
                    subtotal + delivery_fee + service_fee + tax_amount - discount_amount
                )
                ...
                order = {
                    ...
                    "delivery_instructions": self._generate_delivery_instructions()
                    if random.random() > 0.6
                    else None,
                    "order_date": order_date,
                    "estimated_preparation_time": random.randint(15, 30),
                    "estimated_delivery_time": random.randint(30, 60),
                    "requested_delivery_time": order_date
                    + timedelta(minutes=random.randint(45, 120))
                    if random.random() < 0.2
                    else None,
                    "status": status,
                    ...
                    "payment_transaction_id": f"TXN{random.randint(1000000000, 9999999999)}"
                    if status != OrderStatus.PENDING
                    else None,
                    "promotion_codes": [f"PROMO{random.randint(100, 999)}"]
                    if random.random() < 0.25
                    else [],
                    "loyalty_points_used": random.randint(0, 500)
                    if random.random() < 0.15
                    else 0,
                    "loyalty_points_earned": int(total_amount / 1000)
                    if status == OrderStatus.DELIVERED
                    else 0,  # 1 point per 1000 VND
                    "contact_preference": random.choice(["phone", "app", "sms"]),
                    "special_instructions": self._generate_special_instructions()
                    if random.random() > 0.7
                    else None,
                    "customer_rating": random.choice(list(ReviewRating))
                    if status == OrderStatus.DELIVERED and random.random() > 0.4
                    else None,
                    "customer_feedback": self._generate_customer_feedback()
                    if random.random() > 0.8
                    else None,
                    "created_at": order_date,
                    "updated_at": order_date + timedelta(minutes=random.randint(5, 120))
                    if random.random() > 0.3
                    else None,
                }
    ...
    def seed_deliveries(
        self, count: int, order_ids: List[ObjectId], rider_ids: List[ObjectId]
    ):
        ...
        for order_id in selected_order_ids:
            ...
            delivery = {
                ...
                if status
                not in [DeliveryStatus.ASSIGNED, DeliveryStatus.EN_ROUTE_TO_RESTAURANT]
                else None,
                "delivered_at": delivery_time
                if status == DeliveryStatus.DELIVERED
                else None,
                "estimated_delivery_time": assigned_at
                + timedelta(minutes=random.randint(45, 75)),
                "status": status,
                "current_rider_location": self._generate_random_coordinates()
                if status not in [DeliveryStatus.DELIVERED, DeliveryStatus.FAILED]
                else None,
                "location_updates": self._generate_location_updates(
                    assigned_at, status
                ),
                ...
                if status == DeliveryStatus.DELIVERED and random.random() > 0.3
                else None,
                "delivery_notes": self._generate_delivery_notes()
                if random.random() > 0.7
                else None,
                "issues_reported": self._generate_delivery_issues()
                if status == DeliveryStatus.FAILED or random.random() < 0.1
                else [],
                "resolution_notes": "Issue resolved successfully"
                if random.random() > 0.8
                else None,
                "actual_delivery_time_minutes": random.randint(25, 60)
                if status == DeliveryStatus.DELIVERED
                else None,
                "customer_rating": random.choice(list(ReviewRating))
                if status == DeliveryStatus.DELIVERED and random.random() > 0.4
                else None,
                "rider_efficiency_score": random.uniform(75, 100)
                if status == DeliveryStatus.DELIVERED
                else None,
                "created_at": assigned_at,
                "updated_at": delivery_time
                if status == DeliveryStatus.DELIVERED
                else assigned_at + timedelta(minutes=random.randint(5, 60)),
            }
    ...
    def seed_payments(
        self, count: int, order_ids: List[ObjectId], customer_ids: List[ObjectId]
    ):
        ...
        for i, order_id in enumerate(random.choices(order_ids, k=count)):
            ...
            initiated_at = self.fake.date_time_between(
                start_date="-60d", end_date="now"
            )
            ...
            payment = {
                ...
                "external_transaction_id": f"EXT{random.randint(1000000, 9999999)}"
                if payment_method
                in [PaymentMethod.MOMO, PaymentMethod.ZALOPAY, PaymentMethod.GRABPAY]
                else None,
                "status": status,
                "initiated_at": initiated_at,
                "completed_at": initiated_at
                + timedelta(seconds=random.randint(10, 300))
                if status == PaymentStatus.COMPLETED
                else None,
                "card_last_four": str(random.randint(1000, 9999))
                if payment_method
                in [PaymentMethod.CREDIT_CARD, PaymentMethod.DEBIT_CARD]
                else None,
                "wallet_type": payment_method.value
                if payment_method
                in [PaymentMethod.MOMO, PaymentMethod.ZALOPAY, PaymentMethod.GRABPAY]
                else None,
                ...
                if payment_method != PaymentMethod.CASH
                else 0,  # 2.5% processing fee
                "platform_fee": amount * 0.02,  # 2% platform fee
                "refund_amount": amount * random.uniform(0.5, 1.0)
                ...
                "refunded_at": initiated_at + timedelta(days=random.randint(1, 7))
                if status == PaymentStatus.REFUNDED
                else None,
                ...
                and random.choice([True] * 9 + [False]),
                "settlement_date": initiated_at + timedelta(days=random.randint(1, 3))
                if status == PaymentStatus.COMPLETED
                else None,
                "settlement_amount": amount * 0.95
                if status == PaymentStatus.COMPLETED
                else None,  # 95% after fees
                "created_at": initiated_at,
                "updated_at": initiated_at + timedelta(minutes=random.randint(1, 30))
                if random.random() > 0.4
                else None,
            }
    ...
    def seed_reviews(
        ...
        for i in range(count):
            ...
            review = {
                ...
                "food_rating": random.choice(list(ReviewRating))
                if restaurant_id
                else None,
                "service_rating": random.choice(list(ReviewRating)),
                "delivery_rating": random.choice(list(ReviewRating)),
                "title": self._generate_review_title(overall_rating)
                if random.random() > 0.4
                else None,
                "comment": self._generate_review_comment(overall_rating)
                if random.random() > 0.3
                else None,
                "photos": [
                ...
                "business_response": self._generate_business_response()
                if restaurant_id and random.random() > 0.6
                else None,
    ...
    def seed_promotions(
        self, count: int, city_ids: List[ObjectId], restaurant_ids: List[ObjectId]
    ):
        ...
        for i in range(count):
            promotion_name = (
                f"{random.choice(promotion_names)} {random.randint(1, 100)}"
            )
            ...
            promotion = {
                ...
                "applicable_cities": random.sample(
                    city_ids, k=random.randint(1, len(city_ids))
                )
                if random.random() > 0.3
                else [],
                "applicable_restaurants": random.sample(
                    restaurant_ids, k=random.randint(0, min(10, len(restaurant_ids)))
                )
                if random.random() > 0.7
                else [],
                ...
                "new_customers_only": random.choice(
                    [True] * 3 + [False] * 7
                ),  # 30% new customer only
            ...
            addresses.append(
                {
                    ...
                    "city": random.choice(
                        [city["name"] for city in self.vietnamese_cities]
                    ),
                    "coordinates": {
    ...
    def _generate_coordinates_near_city(
        self, city_id: ObjectId, city_ids: List[ObjectId]
    ) -> Dict[str, float]:
    ...
    def _generate_item_description(self, dish_name: str, category: str) -> str:
        ...
        return descriptions.get(
            dish_name,
            f"Delicious {dish_name.lower()} made with fresh ingredients and authentic Vietnamese flavors. Perfect for lunch or dinner.",
        )
    ...
    def _generate_ingredients(self, dish_name: str) -> List[str]:
        ...
        return common_ingredients.get(
            dish_name, ["rice", "vegetables", "herbs", "fish sauce", "garlic", "onions"]
        )
    ...
    def _generate_random_coordinates(
        self, city_ids: List[ObjectId] = None
    ) -> Dict[str, float]:
        ...
        for i in range(count):
            ...
            item = {
                ...
                "name": random.choice(
                    [
                        dish
                        for dishes in self.vietnamese_dishes.values()
                        for dish in dishes
                    ]
                ),
                "base_price": base_price,
    ...
    def _generate_delivery_address(self) -> Dict[str, str]:
        ...
        return {
            ...
            "landmark": random.choice(
                ["Near ABC Mall", "Opposite XYZ School", "Next to Coffee Shop"]
            )
            if random.random() > 0.6
            else None,
        }
    ...
    def _generate_status_history(
        self, current_status: OrderStatus, order_date: datetime
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_location_updates(
        self, start_time: datetime, status: DeliveryStatus
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_delivery_issues(self) -> List[Dict[str, Any]]:
        ...
        issues = [
            ...
            },
            {
                ...
                "description": "Customer did not answer phone calls",
            },
            {
                "type": "vehicle_breakdown",
                "description": "Motorcycle had technical issues",
            },
            {
                "type": "weather_delay",
                "description": "Heavy rain caused delivery delay",
            },
    ...
    def _generate_promotion_description(
        self, promo_type: PromotionType, value: float
    ) -> str:
        ...
        return descriptions.get(
            promo_type, "Special promotion available for limited time!"
        )
    ...
    def _validate_referential_integrity(self):
        ...
        if invalid_customer_cities > 0:
            raise ValueError(
                f"Found {invalid_customer_cities} customers with invalid city references"
            )

        # Check restaurant references
        restaurant_ids = set(
            doc["_id"] for doc in self.db.restaurants.find({}, {"_id": 1})
        )
        ...
        if invalid_order_restaurants > 0:
            raise ValueError(
                f"Found {invalid_order_restaurants} orders with invalid restaurant references"
            )
    ...
    def _validate_data_distribution(self):
        ...
        if total_orders > 0:
            delivered_orders = self.db.orders.count_documents(
