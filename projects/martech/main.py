...
def generate_summary_report(connection_string: str):
    ...
    try:
        ...
        sample_customer = db.customers.find_one({}, {
            ...
            'lifetime_value': 1, 'engagement_score': 1, 'tags': 1
        })
        if sample_customer:
        ...
        active_campaigns = list(db.campaigns.find({'status': 'active'}, {'name': 1, 'metrics': 1}).limit(3))
        if active_campaigns:
            print(f"\n📢 Active Campaigns:")
            for campaign in active_campaigns:
                metrics = campaign.get('metrics', {})
                if metrics:
                    print(f"  {campaign['name']} - Sent: {metrics.get('sent', 0):,}, Opened: {metrics.get('opened', 0):,}")
