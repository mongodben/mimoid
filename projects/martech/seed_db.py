...
from db_schema import database_schema, EventType, CampaignType, CampaignStatus, DataSourceType
...
class DataTechPlatformSeeder(DatabaseSeeder):
    ...
    def seed_data_sources(self, count: int):
        ...
        for i in range(count):
            ...
            source = {
                ...
                'last_sync': self.fake.date_time_between(start_date='-7d', end_date='now'),
    ...
    def seed_customers(self, count: int):
        ...
        for i in range(count):
            ...
            lifetime_value = max(0, random.expovariate(1/500))  # Exponential distribution
            ...
            if random.random() < 0.5:  # 50% have company size
                custom_fields['company_size'] = random.choice(['1-10', '11-50', '51-200', '201-1000', '1000+'])
            ...
            customer = {
                ...
                'external_id': f"ext_{self.fake.uuid4()[:8]}" if random.random() < 0.4 else None,
                ...
                'gender': random.choice(['male', 'female', 'other', 'prefer_not_to_say']) if random.random() < 0.4 else None,
                'location': {
                    'city': self.fake.city(),
                ...
                'tags': random.sample(['vip', 'lead', 'customer', 'prospect', 'churned', 'high-value', 'new'], 
                                    k=random.randint(1, 4)),
                'lifetime_value': round(lifetime_value, 2),
                'engagement_score': round(engagement_score, 1),
                'last_activity_date': self.fake.date_time_between(start_date='-90d', end_date='now') if random.random() < 0.8 else None,
                'preferences': {
                ...
                'data_sources': random.sample([str(ds_id) for ds_id in self.data_source_ids], k=random.randint(1, 3))
            }
    ...
    def seed_campaigns(self, count: int):
        ...
        campaign_names = [
            "Welcome Series", "Product Launch", "Holiday Sale", "Re-engagement Campaign",
            "Newsletter", "Product Demo", "Webinar Promotion", "Customer Survey",
            "Loyalty Program", "Referral Campaign", "Abandoned Cart", "Win-back Campaign"
        ]
        ...
        for i in range(count):
            ...
            end_date = start_date + timedelta(days=random.randint(7, 90)) if random.random() < 0.8 else None
            ...
            campaign = {
                ...
                'status': status.value,
                'content': {
                    'subject': self.fake.sentence(nb_words=6) if campaign_type == CampaignType.EMAIL else None,
                    'body': self.fake.text(max_nb_chars=500),
                    'cta_text': random.choice(['Learn More', 'Shop Now', 'Sign Up', 'Get Started']),
                    'template_id': f"template_{random.randint(1, 20)}"
                },
                'targeting': {
                    'segments': random.sample([f"segment_{i}" for i in range(1, 11)], k=random.randint(1, 3)),
                    'criteria': {
                        'engagement_score': {'$gte': random.randint(20, 60)},
                        'tags': {'$in': random.sample(['lead', 'customer', 'vip'], k=random.randint(1, 2))}
                    }
                },
                ...
                'updated_at': self.fake.date_time_between(start_date='-30d', end_date='now') if random.random() < 0.7 else None
            }
        ...
        for batch in range(0, count, batch_size):
            ...
            for i in range(batch, batch_end):
                ...
                if event_type_choice == EventType.PAGE_VIEW:
                    ...
                    properties = {
                        'email_id': str(random.choice(self.campaign_ids)) if self.campaign_ids else None,
                ...
                event = {
                    ...
                    'session_id': f"session_{self.fake.uuid4()[:8]}" if random.random() < 0.7 else None,
                    'campaign_id': random.choice(self.campaign_ids) if random.random() < 0.2 and self.campaign_ids else None,
                    'user_agent': self.fake.user_agent() if random.random() < 0.8 else None,
                    'ip_address': self.fake.ipv4() if random.random() < 0.6 else None,
                    'device_type': random.choice(['desktop', 'mobile', 'tablet']),
                    'page_url': self.fake.url() if event_type_choice == EventType.PAGE_VIEW else None,
    ...
    def seed_segments(self, count: int):
        ...
        segment_templates = [
            {'name': 'High Value Customers', 'criteria': {'lifetime_value': {'$gte': 1000}}},
            {'name': 'Recent Purchasers', 'criteria': {'tags': {'$in': ['customer']}, 'last_activity_date': {'$gte': datetime.now() - timedelta(days=30)}}},
            {'name': 'Email Engaged', 'criteria': {'engagement_score': {'$gte': 60}}},
            {'name': 'Mobile Users', 'criteria': {'preferences.push_notifications': True}},
            {'name': 'Enterprise Prospects', 'criteria': {'custom_fields.company_size': {'$in': ['201-1000', '1000+']}}},
        ]
        ...
        for i in range(count):
            if i < len(segment_templates):
                ...
                criteria = {
                    ...
                    'tags': {'$in': random.sample(['lead', 'customer', 'vip', 'prospect'], k=random.randint(1, 2))}
                }
            ...
            segment = {
                ...
                'last_calculated': self.fake.date_time_between(start_date='-7d', end_date='now'),
                'is_dynamic': random.choice([True, True, False]),  # 67% dynamic
                'created_by': f"user_{random.randint(1, 10)}",
                'created_at': self.fake.date_time_between(start_date='-1y', end_date='-30d'),
                'updated_at': self.fake.date_time_between(start_date='-30d', end_date='now') if random.random() < 0.5 else None
            }
    ...
    def seed_social_members(self, count: int):
        ...
        social_customers = random.sample(self.customer_ids, k=min(count, len(self.customer_ids) // 3))
        ...
        for i, customer_id in enumerate(social_customers):
            ...
            for platform in customer_platforms:
                member = {
                    ...
                    'last_activity': self.fake.date_time_between(start_date='-30d', end_date='now'),
                    'connected_at': self.fake.date_time_between(start_date='-2y', end_date='-30d'),
                    'is_active': random.choice([True, True, True, False])  # 75% active
                }
    ...
    def validate_seed_data(self):
        ...
        try:
            ...
            if invalid_events > 0:
                ...
                validation_results['errors'].append(f"Found {invalid_events} events with invalid customer references")
            ...
            if invalid_social > 0:
                validation_results['referential_integrity'] = False
                validation_results['errors'].append(f"Found {invalid_social} social members with invalid customer references")
        ...
        for collection_name, collection_schema in self.database_schema.collections.items():
            ...
            for index_def in collection_schema.indexes:
                if index_def.name not in existing_indexes and index_def.name not in failed_unique_indexes:
