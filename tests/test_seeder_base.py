...
from mimiod import DatabaseSeeder, BaseMongoDbSchema, BaseCollectionSchema, IndexDefinition, IndexDirection
...
class TestValidateSchemaAndIndexes:
        ...
        users_collection = BaseCollectionSchema(
            ...
            indexes=[
                ...
                IndexDefinition(
                    ...
                    keys={"age": IndexDirection.ASCENDING, "active": IndexDirection.DESCENDING},
                    unique=False
                )
            ]
        )
        ...
        products_collection = BaseCollectionSchema(
            ...
            indexes=[
                IndexDefinition(
                    ...
                    keys={"product_name": IndexDirection.ASCENDING, "category": IndexDirection.ASCENDING},
                    unique=False
                )
            ]
        )
    ...
    @patch('mimiod.seeder_base.MongoClient')
    def test_validate_schema_and_indexes_success(self, mock_mongo_client, concrete_seeder, sample_pydantic_models):
        ...
        mock_users_collection.list_indexes.return_value = [
            ...
            {"name": "age_active_index", "key": {"age": 1, "active": -1}, "unique": False, "sparse": False}
        ]
        mock_products_collection.list_indexes.return_value = [
            {"name": "_id_", "key": {"_id": 1}},
            {"name": "product_name_category", "key": {"product_name": 1, "category": 1}, "unique": False, "sparse": False}
        ]
        
        # Run validation
        result = concrete_seeder.validate_schema_and_indexes(
            sample_size=10,
            pydantic_models=sample_pydantic_models
        )
        ...
        assert result["collection_results"]["users"]["schema_validation"]["passed"] is True
        assert result["collection_results"]["users"]["index_validation"]["passed"] is True
        assert result["collection_results"]["products"]["schema_validation"]["passed"] is True
        assert result["collection_results"]["products"]["index_validation"]["passed"] is True
    
    @patch('mimiod.seeder_base.MongoClient')
    def test_validate_schema_validation_failure(self, mock_mongo_client, concrete_seeder, sample_pydantic_models):
        ...
        mock_users_collection.find.return_value = [
            {"name": "John Doe", "email": "john@example.com", "age": 30, "active": True},  # Valid
            {"name": "", "email": "invalid", "age": -5, "active": "not_boolean"}  # Invalid
        ]
        ...
        result = concrete_seeder.validate_schema_and_indexes(
            sample_size=10,
            pydantic_models={"users": sample_pydantic_models["users"]}
        )
        
        # Verify results
        assert result["validation_summary"]["overall_success"] is False
        assert result["collection_results"]["users"]["schema_validation"]["passed"] is False
        assert result["collection_results"]["users"]["schema_validation"]["valid_documents"] == 1
        assert result["collection_results"]["users"]["schema_validation"]["invalid_documents"] == 1
        assert len(result["collection_results"]["users"]["schema_validation"]["errors"]) > 0
    
    @patch('mimiod.seeder_base.MongoClient')
    def test_validate_missing_indexes(self, mock_mongo_client, concrete_seeder, sample_pydantic_models):
        ...
        # Verify results
        assert result["validation_summary"]["overall_success"] is False
        assert result["collection_results"]["users"]["index_validation"]["passed"] is False
        assert len(result["collection_results"]["users"]["index_validation"]["missing_indexes"]) == 2
        assert "email_unique" in result["collection_results"]["users"]["index_validation"]["missing_indexes"]
        assert "age_active_index" in result["collection_results"]["users"]["index_validation"]["missing_indexes"]
    
    @patch('mimiod.seeder_base.MongoClient')
    def test_validate_index_property_mismatch(self, mock_mongo_client, concrete_seeder, sample_pydantic_models):
        ...
        mock_users_collection.list_indexes.return_value = [
            {"name": "_id_", "key": {"_id": 1}},
            {"name": "email_unique", "key": {"email": 1}, "unique": False, "sparse": False},  # Should be unique=True
            {"name": "age_active_index", "key": {"age": 1, "active": -1}, "unique": False, "sparse": False}
        ]
        ...
        assert result["validation_summary"]["overall_success"] is False
        assert result["collection_results"]["users"]["index_validation"]["passed"] is False
        assert any("unique mismatch" in error for error in result["collection_results"]["users"]["index_validation"]["errors"])
    
    @patch('mimiod.seeder_base.MongoClient')
    def test_validate_large_collection_sampling(self, mock_mongo_client, concrete_seeder, sample_pydantic_models):
        ...
        mock_users_collection.aggregate.assert_called_once_with([{"$sample": {"size": 2}}])
    ...
    def test_validate_without_pydantic_models(self, mock_mongo_client, concrete_seeder):
        ...
        result = concrete_seeder.validate_schema_and_indexes(sample_size=10, pydantic_models=None)
        
        # Verify results - schema validation should be skipped
        assert result["collection_results"]["users"]["documents_sampled"] == 0
        assert result["collection_results"]["users"]["schema_validation"]["passed"] is True  # Default true when not tested
        assert result["collection_results"]["users"]["index_validation"]["passed"] is True
        assert result["validation_summary"]["schema_validation_passed"] == 2  # Both collections pass by default
    
    @patch('mimiod.seeder_base.MongoClient')
    def test_validate_connection_failure(self, mock_mongo_client, concrete_seeder, sample_pydantic_models):
    ...
    @patch('mimiod.seeder_base.MongoClient')
    def test_validate_extra_indexes_noted(self, mock_mongo_client, concrete_seeder, sample_pydantic_models):
        ...
        mock_users_collection.list_indexes.return_value = [
            ...
            {"name": "email_unique", "key": {"email": 1}, "unique": True, "sparse": False},
            {"name": "age_active_index", "key": {"age": 1, "active": -1}, "unique": False, "sparse": False},
            {"name": "extra_name_index", "key": {"name": 1}, "unique": False, "sparse": False}  # Extra
        ]
        ...
        assert result["collection_results"]["users"]["index_validation"]["passed"] is True
        assert result["collection_results"]["products"]["index_validation"]["passed"] is True
        assert "extra_name_index" in result["collection_results"]["users"]["index_validation"]["extra_indexes"]